...
module.exports.getMarket = function (
  key,
  percent,
  changePriseInterval,
  blackList,
  ...
  this._changePriseInterval = changePriseInterval;
  this._msg = msg;
  ...
  this._UpdateInventory = () => {
    request.loading(
      "https://market.csgo.com/api/UpdateInventory/?key=" + this._key,
      (data) => {
        this._JsonLog(data);
      }
    );
  ...
  this._GetChangePrice = (id, cost) => {
    queue.queuePush((cb) => {
      const newPrice =
        cost > 100
          ? Math.round(cost * (1 - this._percent / 100)) * 100
          : (cost - 1) * 100;
      request.loading(
        "https://market.csgo.com/api/v2/set-price?key=" +
          this._key +
          "&item_id=" +
          id +
          "&price=" +
  ...
  this._GetItems = () => {
    request.loading(
      "https://market.csgo.com/api/v2/items?key=" + this._key,
      (data) => {
  ...
  this._GetMoney = () => {
    request.loading(
      "https://market.csgo.com/api/GetMoney/?key=" + this._key,
      (data) => {
        if (data.money != this._balance && !Number.isNaN(data.money)) {
          this._balance = data.money;
          this._msg(
            "Денежный баланс на сайте: " + Math.floor(data.money / 100),
            0
          );
        }
      }
  ...
  this._SetPrice = (id, cost) => {
    request.loading(
      "https://market.csgo.com/api/v2/add-to-sale?key=" +
        this._key +
        "&id=" +
        id +
        "&price=" +
      ...
      (data) => {
        if (data.success) {
          this._JsonLog(
            "Предмет " + id + " выставлен на продажу за " + cost + "руб."
          );
        } else {
          ...
          if (data.error == "no_description_found") {
            this._JsonLog(
              "Steam не вернул описание предмета " + id + " попробуйте позже"
            );
          }
          ...
          if (data.error == "item_not_in_inventory") {
            this._JsonLog(
              "Предмет " +
                id +
                " не найден в инвентаре, попробуйте сначала обновить его с помощью метода UpdateInventory и подождать 10-20 секунд перед повторной попыткой."
            );
            this._UpdateInventory();
          }
  ...
  this._BuyOffers = (item) => {
    queue.queuePush((cb) => {
      ...
      request.loading(
        "https://market.csgo.com/api/v2/get-list-items-info?key=" +
          key +
          "&list_hash_name[]=" +
          encoded,
        (data) => {
  ...
  this._GetInv = () => {
    request.loading(
      "https://market.csgo.com/api/v2/my-inventory/?key=" + this._key,
      (data) => {
        if (data.success && data.items && data.items.length > 0) {
          data.items.forEach((item) => {
  ...
  this._GetItemsToGive = () => {
    request.loading(
      "https://market.csgo.com/api/v2/trade-request-give-p2p-all?key=" +
        this._key,
      (data) => {
  ...
  this._Test = () => {
    request.loading(
      "https://market.csgo.com/api/v2/test?key=" + this._key,
      (data) => {
        if (
          data.success &&
          data.status.user_token &&
          data.status.trade_check &&
          data.status.site_online &&
  ...
  this._Online = () => {
    request.loading(
      "https://market.csgo.com/api/v2/ping?key=" + this._key,
      (data) => {
        if (data.success) {
